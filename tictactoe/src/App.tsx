...
const Game: React.FC = () => {
  ...
  const [selectedPlayerBoards, setSelectedPlayerBoards] = useState<
    Array<string>
  >([]);
  ...
  const getAllPlayerGames = async () => {
    ...
    const selectedPlayerBoardsRes = result.filter(
      (res: any) => res.name === selectedPlayer
    )[0].boards;
  ...
  return (
    <>
      ...
      />
      <div style={{ position: "fixed", top: 10, left: "45%" }}>
        {fetchResultState}
      </div>
      <div>
        <SquareLine
        ...
        <>
          {selectedPlayerBoards.map((board) => {
            return (
              <div onClick={(_) => setSquares(JSON.parse(board))}>{board}</div>
            );
          })}
        </>
      </div>
...
const SquareLine: React.FC<SquareLineProps> = (props) => {
  ...
  return (
    ...
    >
      {squareLine.map((square, colIndex) => (
        <Square
          setSquare={setSquare}
          content={square}
          index={rowIndex * 3 + colIndex}
          key={`square-line-${rowIndex * 3 + colIndex}`}
        />
      ))}
    </div>
  );
